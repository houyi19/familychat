...
import com.study.familychat.tool.adapter.CustomDateAdapter;
...
import com.study.familychat.tool.listener.OnMultiChooseListener;
...
public class CustomeCalendarView extends ViewPager {

    //记录当前adapter所指的position位置
    ...
    private CustomDateAdapter mCustomDateAdapter;
    ...
    public CustomeCalendarView(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);

        //初始化属性类
        mAttrsBean = new AttrsBean();

        //TODO 自定义属性还没有设置完成，先提交一次；
        //获取自定义的view属性；
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.CustomeCalendarView);
...
//        根据设定的日期范围计算日历的页数;
        ...
        mCustomDateAdapter.setmAttrsBean(mAttrsBean);
        mCustomDateAdapter.setCalendarViewAdapter(item_layout, calendarViewAdapter);
        setAdapter(mCustomDateAdapter);

        mCurrentPosition = CalendarUtil.dateToPosition(initDate[0], initDate[1], startDate[0], startDate[1]);
    ...
    private void refreshMonthView(int position) {
        MonthView monthView = mCustomDateAdapter.getmViews().get(position);
    ...
    public void setLastClickDay(int day) {
        lastClickDate[0] = mCurrentPosition;
        lastClickDate[1] = day;
    }
     ...
     */
    public CustomeCalendarView setOnCalendarViewAdapter(int item_layout, CalendarViewAdapter calendarViewAdapter) {
    ...
    public void today() {
        ...
        if (destPosition == mCurrentPosition) {
    ...
    public void lastYear() {
        if (mCurrentPosition - 12 >= 0) {
            setCurrentItem(mCurrentPosition -= 12, false);
        }
    }
     ...
     * 将指定日期的农历替换成对应文字
     */
    public CustomeCalendarView setSpecifyMap(HashMap<String, String> map) {
     ...
     * @return
     */
    public CustomeCalendarView setInitDate(String date) {
     ...
     * @return
     */
    public CustomeCalendarView setStartEndDate(String startDate, String endDate) {
     ...
     * @return
     */
    public CustomeCalendarView setMultiDate(List<String> dates) {
     ...
     * @return
     */
    public CustomeCalendarView setSingleDate(String date) {
     ...
     * @return
     */
    public CustomeCalendarView setDisableStartEndDate(String startDate, String endDate) {
